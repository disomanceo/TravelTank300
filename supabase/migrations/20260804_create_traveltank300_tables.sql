...
create index if not exists travel_places_user_visit_date_idx
  on public.travel_places (user_id, visit_date desc);
create index if not exists travel_places_user_created_at_idx
  on public.travel_places (user_id, created_at desc);
create index if not exists travel_places_province_idx
  on public.travel_places (province);
...
create index if not exists travel_place_photos_place_sort_idx
  on public.travel_place_photos (place_id, sort_order);
create unique index if not exists travel_place_photos_one_cover_idx
  on public.travel_place_photos (place_id)
  where is_cover = true;
...
create table if not exists public.travel_plans (
  ...
  status text not null default 'planning'
    check (status in ('idea','planning','ready','travelling','completed','cancelled')),
  ...
  created_at timestamptz not null default timezone('utc', now()),
  updated_at timestamptz not null default timezone('utc', now()),
  constraint travel_plans_date_order check (
    start_date is null or end_date is null or end_date >= start_date
  )
);

create index if not exists travel_plans_user_start_date_idx
  on public.travel_plans (user_id, start_date desc nulls last);
create index if not exists travel_plans_user_created_at_idx
...
create index if not exists travel_plan_stops_plan_order_idx
  on public.travel_plan_stops (plan_id, visit_order);
...
create index if not exists travel_plan_photos_plan_sort_idx
  on public.travel_plan_photos (plan_id, sort_order);
create unique index if not exists travel_plan_photos_one_cover_idx
  on public.travel_plan_photos (plan_id)
  where is_cover = true;

drop trigger if exists travel_places_set_updated_at on public.travel_places;
create trigger travel_places_set_updated_at
...
create trigger travel_plan_stops_set_updated_at
before update on public.travel_plan_stops
for each row execute function public.travel_set_updated_at();
...
create policy "travel_place_photos_select_own" on public.travel_place_photos
for select to authenticated using (
  exists (select 1 from public.travel_places p where p.id = place_id and p.user_id = auth.uid())
);
drop policy if exists "travel_place_photos_insert_own" on public.travel_place_photos;
create policy "travel_place_photos_insert_own" on public.travel_place_photos
for insert to authenticated with check (
  exists (select 1 from public.travel_places p where p.id = place_id and p.user_id = auth.uid())
);
drop policy if exists "travel_place_photos_update_own" on public.travel_place_photos;
create policy "travel_place_photos_update_own" on public.travel_place_photos
...
create policy "travel_place_photos_delete_own" on public.travel_place_photos
...
create policy "travel_plan_stops_select_own" on public.travel_plan_stops
for select to authenticated using (
  exists (select 1 from public.travel_plans p where p.id = plan_id and p.user_id = auth.uid())
);
drop policy if exists "travel_plan_stops_insert_own" on public.travel_plan_stops;
create policy "travel_plan_stops_insert_own" on public.travel_plan_stops
for insert to authenticated with check (
  exists (select 1 from public.travel_plans p where p.id = plan_id and p.user_id = auth.uid())
);
drop policy if exists "travel_plan_stops_update_own" on public.travel_plan_stops;
create policy "travel_plan_stops_update_own" on public.travel_plan_stops
for update to authenticated using (
  exists (select 1 from public.travel_plans p where p.id = plan_id and p.user_id = auth.uid())
) with check (
  exists (select 1 from public.travel_plans p where p.id = plan_id and p.user_id = auth.uid())
...
create policy "travel_plan_stops_delete_own" on public.travel_plan_stops
for delete to authenticated using (
  exists (select 1 from public.travel_plans p where p.id = plan_id and p.user_id = auth.uid())
);

drop policy if exists "travel_plan_photos_select_own" on public.travel_plan_photos;
create policy "travel_plan_photos_select_own" on public.travel_plan_photos
for select to authenticated using (
  exists (select 1 from public.travel_plans p where p.id = plan_id and p.user_id = auth.uid())
);
drop policy if exists "travel_plan_photos_insert_own" on public.travel_plan_photos;
create policy "travel_plan_photos_insert_own" on public.travel_plan_photos
for insert to authenticated with check (
  exists (select 1 from public.travel_plans p where p.id = plan_id and p.user_id = auth.uid())
);
drop policy if exists "travel_plan_photos_update_own" on public.travel_plan_photos;
create policy "travel_plan_photos_update_own" on public.travel_plan_photos
for update to authenticated using (
  exists (select 1 from public.travel_plans p where p.id = plan_id and p.user_id = auth.uid())
) with check (
  exists (select 1 from public.travel_plans p where p.id = plan_id and p.user_id = auth.uid())
...
create policy "travel_plan_photos_delete_own" on public.travel_plan_photos
for delete to authenticated using (
  exists (select 1 from public.travel_plans p where p.id = plan_id and p.user_id = auth.uid())
);
